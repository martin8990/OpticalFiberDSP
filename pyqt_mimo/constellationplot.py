...
class ConstellationPlot(MimoFigure):
    ...
    def _getcolors(self,sig_bitsep):
        colors= cm.rainbow(np.linspace(0, 1, len(sig_bitsep)))
        return colors[:,:3]
    def __init__(self,sig_bitsep,nsyms,name):
        self.sig_bitsep = sig_bitsep
        self.colors = self._getcolors(sig_bitsep)
        self.nsyms= nsyms
        self.name = name
    
    def make_constelation_plot(self,figure,i_bit):
        sig = self.sig_bitsep[i_bit]
    ...
    def create_figure(self,win):
        ...
        self.list_plots = []
        for i_bit in range(len(self.sig_bitsep)):
    ...
    def update_region(self, r_min, r_max):
        for i_bit in range(len(self.list_plots)):
            len_arr = len(self.sig_bitsep[i_bit])
            bitmin = int(r_min/self.nsyms *len_arr)
            bitmax = int(r_max/self.nsyms * len_arr)
            self.list_plots[i_bit].setData(self.sig_bitsep[i_bit].real[bitmin:bitmax],self.sig_bitsep[i_bit].imag[bitmin:bitmax])               
