...
#list_delay = [500e-12,510e-12,520e-12,530e-12,540e-12,550e-12,560e-12,570e-12,580e-12,590e-12]
list_pmd =  [1e-9]
list_snr = [10,13,15]
list_mu_m = [2e-4,3e-4]
list_mu_q = []
...
list_lb = [64,80,128,256,512,1024]
list_ntaps = [21,61,81]
R2 = 1
phase_noise = 100e3 
...
for pmd in list_pmd:
    for snr in list_snr:
        sigdir = create_dir_for_signal(testDir,pmd,snr,apply_impulse,ovsmpl)
        for r in range(nrepetitions):
            ...
            if apply_matrix:
                if delay_after_matrix:
                    ...
                    sig[:,:] = apply_2chnl_delayed_after_mix_matrix_impairment(sig,1e-9,25e9)
            ...
            for mu_m in list_mu_m:
                 for lb in list_lb:
                    ...
                    errorcalc = TrainedLMS(trainingSyms,constellation,10000,lb)
                    mimo = FrequencyDomainBlockwizeMimo(2,lb,ovsmpl,mu_m,errorcalc)
