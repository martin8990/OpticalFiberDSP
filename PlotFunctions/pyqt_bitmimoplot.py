...
class ConstellationPlot(MimoFigure):
    ...
    def _getcolors(self,bit_sigs):
        ...
        return colors[:,:3]
    def __init__(self,sig_bitsep,nsyms):
    ...
    def create_figure(self,name):
        self.figure = win.addPlot(title= name +" Constellation")
        self.list_plots = []
    ...
    def update_region(self, r_min, r_max):
        for i_bit in range(len(self.list_plots)):
            len_arr = len(self.bit_sigs[i_bit])
            bitmin = int(r_min/self.nsyms *len_arr)
            bitmax = int(r_max/self.nsyms * len_arr)
            self.list_constellation_plots[i_bit].setData(self.bit_sigs[i_bit].real[bitmin:bitmax],self.bit_sigs[i_bit].imag[bitmin:bitmax])               
