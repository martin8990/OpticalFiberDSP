    ...
    for i_input in range(nmodes):
        for i_output in range(nmodes):        
            ...
            for i_ovsmpl in range(ovsmpl):
                FD_temp = FD_temp + double_block_fd[i_input,i_ovsmpl] * H[i_input,i_output,i_ovsmpl]
            block_compensated[i_output] = block_compensated[i_output] + np.fft.ifft(FD_temp)[second_block]
